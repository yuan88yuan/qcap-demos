...
namespace __test_demuxer__ {
	...
	// inline float mode_vrefresh(drmModeModeInfo *mode) { // Not directly used by TestCase3
	// 	return mode->clock * 1000.00 / (mode->htotal * mode->vtotal);
	// }

	// static uint32_t get_prop_id(int fd, drmModeObjectProperties *props, std::shared_ptr<drmModePropertyRes>* prop_res, const char *name) { // Not directly used by TestCase3
...
struct App0 {
	...
	void DoCase(int t) {
		...
		switch(t) {
		case 1: // Keeping this case, but it will directly call mTestCase3
			mTestCase3.DoWork();
			break;
		case 2: // Keeping this case, but it will directly call mTestCase3
			mTestCase3.DoWork();
			break;
		case 3:
			mTestCase3.DoWork();
