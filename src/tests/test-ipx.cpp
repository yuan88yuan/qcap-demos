	...
	struct TestCase2 : public TestCase {
		...
		QRETURN OnStart(free_stack_t& _FreeStack_, QRESULT& qres) {
			switch(1) { case 1:
				...
				qres = AddEventHandler(_FreeStack_, pEvent_vsrc,
					std::bind(&self_t::OnVsrc, this, pVsrc, pVsca, packed_image_size, pEncoder, pVencQ, pVencEvtQ));
				if(qres != QCAP_RS_SUCCESSFUL) {
		...
		QRESULT StartVsrc(free_stack_t& _FreeStack_, qcap2_event_t* pEvent_vsrc, qcap2_video_source_t** ppVsrc) {
			...
			switch(1) { case 1:
				...
				for(int i = 0;i < nBuffers;i++) {
					...
					qres = __testkit__::new_video_sysbuf(_FreeStack_, nColorSpaceType, nVideoFrameWidth, nVideoFrameHeight, &pRCBuffer);
					if(qres != QCAP_RS_SUCCESSFUL) {
		...
		QRESULT StartVsca(free_stack_t& _FreeStack_, qcap2_video_scaler_t** ppVsca) {
			...
			switch(1) { case 1:
				...
				oNVBufParam.gpuId = 0;
				oNVBufParam.colorFormat = NVBUF_COLOR_FORMAT_UYVY;
				oNVBufParam.width = nVideoFrameWidth;
				oNVBufParam.height = nVideoFrameHeight;
				...
				for(int i = 0;i < nBuffers;i++) {
					...
					qres = qcap2_rcbuffer_map_cures_nvbuf(pRCBuffer, CU_GRAPHICS_MAP_RESOURCE_FLAGS_NONE);
					if(qres != QCAP_RS_SUCCESSFUL) {
						LOGE("%s(%d): qcap2_rcbuffer_map_cures_nvbuf() failed, qres=%d", __FUNCTION__, __LINE__, qres);
						break;
					}
					ZzUtils::Scoped ZZ_GUARD_NAME([pRCBuffer]() {
						QRESULT qres;

						qres = qcap2_rcbuffer_unmap_cures_nvbuf(pRCBuffer);
						if(qres != QCAP_RS_SUCCESSFUL) {
							LOGE("%s(%d): qcap2_rcbuffer_unmap_cures_nvbuf() failed, qres=%d", __FUNCTION__, __LINE__, qres);
						}
					});

					uintptr_t pBuffer[3];
					int pPitch[3];
					qres = qcap2_rcbuffer_get_mapped_eglbuffer(pRCBuffer, pBuffer, pPitch);
					if(qres != QCAP_RS_SUCCESSFUL) {
						LOGE("%s(%d): qcap2_rcbuffer_get_mapped_eglbuffer() failed, qres=%d", __FUNCTION__, __LINE__, qres);
						break;
					}

					LOGD("%d: mapped eglbuffer => [%p,%p,%p] [%d,%d,%d]", i, pBuffer[0], pBuffer[1], pBuffer[2],
						pPitch[0], pPitch[1], pPitch[2]);
#endif
#endif

					qres = __testkit__::new_video_cudabuf_managed(_FreeStack_, nColorSpaceType, nVideoFrameWidth, nVideoFrameHeight, cudaMemAttachGlobal, &pRCBuffer);
					if(qres != QCAP_RS_SUCCESSFUL) {
						LOGE("%s(%d): __testkit__::new_video_cudabuf_managed() failed, qres=%d", __FUNCTION__, __LINE__, qres);
						break;
					}
					...
					LOGD("%d: mapped => [%p,%p,%p,%p] [%d,%d,%d,%d]", i, pData[0], pData[1], pData[2], pData[3],
						pLineSize[0], pLineSize[1], pLineSize[2], pLineSize[3]);
#endif
				...
				qcap2_video_scaler_set_buffers(pVsca, &pBuffers[0]);
				qcap2_video_scaler_set_src_buffer_hint(pVsca, QCAP2_BUFFER_HINT_DEFAULT);
				qcap2_video_scaler_set_dst_buffer_hint(pVsca, QCAP2_BUFFER_HINT_CUDA_MANAGED);
		...
		QRETURN OnVsrc(qcap2_video_source_t* pVsrc, qcap2_video_scaler_t* pVsca, size_t packed_image_size, ipxgpucodec_encoder_t* pEncoder, qcap2_rcbuffer_queue_t* pVencQ, qcap2_rcbuffer_queue_t* pVencEvtQ) {
			QRESULT qres;
			ipxgpucodec_error_t ipxerr;
			int64_t now = _clk();
			...
			switch(1) { case 1:
				...
				qcap2_rcbuffer_t* pRCBuffer0;
				qres = qcap2_video_scaler_pop(pVsca, &pRCBuffer0);
				if(qres != QCAP_RS_SUCCESSFUL) {
					LOGE("%s(%d): qcap2_video_scaler_pop() failed, qres=%d", __FUNCTION__, __LINE__, qres);
					break;
				}
				std::shared_ptr<qcap2_rcbuffer_t> ZZ_GUARD_NAME(pRCBuffer0, qcap2_rcbuffer_release);

				const void* pPackedImage;

#if 0
				qres = qcap2_rcbuffer_map_cures_nvbuf(pRCBuffer0, CU_GRAPHICS_MAP_RESOURCE_FLAGS_NONE);
				if(qres != QCAP_RS_SUCCESSFUL) {
					LOGE("%s(%d): qcap2_rcbuffer_map_cures_nvbuf() failed, qres=%d", __FUNCTION__, __LINE__, qres);
					break;
				}
				ZzUtils::Scoped ZZ_GUARD_NAME([pRCBuffer0]() {
					QRESULT qres;

					qres = qcap2_rcbuffer_unmap_cures_nvbuf(pRCBuffer0);
					if(qres != QCAP_RS_SUCCESSFUL) {
						LOGE("%s(%d): qcap2_rcbuffer_unmap_cures_nvbuf() failed, qres=%d", __FUNCTION__, __LINE__, qres);
					}
				});
				...
				uintptr_t pBuffer[3];
				int pPitch[3];
				qres = qcap2_rcbuffer_get_mapped_eglbuffer(pRCBuffer0, pBuffer, pPitch);
				if(qres != QCAP_RS_SUCCESSFUL) {
					LOGE("%s(%d): qcap2_rcbuffer_get_mapped_eglbuffer() failed, qres=%d", __FUNCTION__, __LINE__, qres);
					break;
				}
				pPackedImage = (const void*)pBuffer[0];
#endif

#if 1
				std::shared_ptr<qcap2_av_frame_t> pAVFrame(
					(qcap2_av_frame_t*)qcap2_rcbuffer_lock_data(pRCBuffer0),
					[pRCBuffer0](qcap2_av_frame_t*) {
						qcap2_rcbuffer_unlock_data(pRCBuffer0);
					});

				uint8_t* pData[4];
				int pLineSize[4];
				qcap2_av_frame_get_buffer1(pAVFrame.get(), pData, pLineSize);
				pPackedImage = (const void*)pData[0];
#endif

#if 1
				// LOGD("packed_image_size=%u", packed_image_size);
				ipxerr = ipxgpucodec_enc_push_frame(pEncoder, pPackedImage, packed_image_size);
				if(ipxerr != IPXGPUCODEC_SUCCESS) {
		...
		QRESULT StartVenc(free_stack_t& _FreeStack_, qcap2_event_t* pEvent_venc, ipxgpucodec_encoder_t** ppEncoder, qcap2_rcbuffer_queue_t** ppVencQ, qcap2_rcbuffer_queue_t** ppVencEvtQ, size_t* pPackedImageSize, uint32_t* pCodeStreamSize) {
			...
			switch(1) { case 1:
				...
				for(int i = 0;i < nBuffers;i++) {
					qcap2_rcbuffer_t* pRCBuffer;
					qres = __testkit__::new_video_cudabuf_managed(_FreeStack_, QCAP_COLORSPACE_TYPE_Y8, codestream_width, codestream_height, cudaMemAttachGlobal, &pRCBuffer);
					if(qres != QCAP_RS_SUCCESSFUL) {
						LOGE("%s(%d): __testkit__::new_video_cudabuf_managed() failed, qres=%d", __FUNCTION__, __LINE__, qres);
						break;
					}
		...
		void Main_Puller(ipxgpucodec_encoder_t* pEncoder, uint32_t codestream_size, qcap2_rcbuffer_queue_t* pVencQ, qcap2_rcbuffer_queue_t* pVencEvtQ) {
			...
			while(true) {
				...
				stats.Log(codestream_size * 8, now);
